...
def le_train(model, device, dataset, optimizer, criterion, epochs, time_step, matrix_d, true_corr, y_s):
    ...
    for i in range(epochs): # looping over epochs
        ...
        one_shot_y = batch_to_full(y_pred, device)
        one_shot_y = one_shot_y.to(device)
        ...
        # Auto-corr Diff Loss
        tau_x, pred_corr = auto_corr(one_shot_y, device)
...
if __name__ == '__main__':
    ...
    N = X.shape[0]
    X_train, X_test, y_train, y_test = train_test_split(N, X, y, partition)
    ...
    legend()
    savefig("ode_ac_whole_plot_"+str(dim)+".png", dpi=300)
